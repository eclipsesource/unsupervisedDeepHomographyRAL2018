    ...
    for i in range(args.img_per_real):
        ...
        for point in four_points:
            perturbed_four_points.append(
                (
                    point[0] + random.randint(-rho, rho),
                    point[1] + random.randint(-rho, rho),
                )
            )
        ...
        try:
            ...
            return index, 0
        inv_warped_color_image = numpy_transformer(
            color_image, H_inverse, (width, height)
        )
        ...
        if args.visual:
            ...
            corr_prime = cv2.perspectiveTransform(
                np.array([[corr]], dtype=np.float32), H_inverse
            )[0, 0]

            patches_visualization = cv2.cvtColor(color_image, cv2.COLOR_BGR2RGB)
            cv2.polylines(
                patches_visualization, np.int32([perturbed_four_points]), 1, (0, 255, 0)
            )
            cv2.polylines(
                patches_visualization, np.int32([four_points]), 1, (0, 0, 255)
            )
            ...
            cv2.polylines(
                patch_warped_visualization, np.int32([four_points]), 1, (0, 0, 0)
            )
            cv2.polylines(
                patch_warped_visualization,
                np.int32([perturbed_four_points]),
                1,
                (0, 255, 0),
            )
            ...
            show_image(
                (2, 2, 4),
                "(I2 warped to I1) vs I1",
                np.abs(np.mean(I2_to_I1, 2) - gray_image),
            )
            plt.show()
            ######################################################################################
...
def _check_existance(file_path: str):
    if Path(file_path).is_file():
        print("-- Current {} existed. Deleting..!".format(file_path))
        os.remove(file_path)


def dataCollection(args):
    if (
        (args.resume == "N" or args.resume == "n")
        and args.start_index == 0
        and args.mode == "train"
        and not args.debug
    ):
        try:
            _check_existance(args.gt_file)
            _check_existance(args.pts1_file)
            _check_existance(args.filenames_file)
    ...
    if (
        ...
        try:
            _check_existance(args.test_gt_file)
            _check_existance(args.test_pts1_file)
            _check_existance(args.test_filenames_file)
...
def main():
    ...
    DATA_NUMBER = 100  # number of pair of synthetic images in training dataset
    TEST_DATA_NUMBER = 40  # number of pair of synthetic images in test dataset
    IM_PER_REAL = 2  # Generate 2 different synthetic images from one single real image

    # Size of synthetic image
    HEIGHT = 240
    WIDTH = 320
    PATCH_SIZE = 128  # Size of crop

    # Directories to files
    RAW_DATA_PATH = "/home/xintian/data/coco/coco_sheep/train/images"  # Real images used for generating synthetic data
    TEST_RAW_DATA_PATH = "/home/xintian/data/coco/coco_sheep/val/images"  # Real images used for generating test synthetic data

    # Synthetic data directories
    DATA_PATH = "/home/xintian/data/coco/coco_sheep/synthetic/" + str(RHO) + "/"
    ...
    parser.add_argument(
        "--color", type=str2bool, default="true", help="Generate color or gray images"
    )
    parser.add_argument(
        "--debug",
        type=str2bool,
        default="false",
        help="Debug Mode. Will not delete or store any thing",
    )

    parser.add_argument(
        "--raw_data_path", type=str, default=RAW_DATA_PATH, help="The raw data path."
    )
    parser.add_argument(
        "--test_raw_data_path",
        type=str,
        default=TEST_RAW_DATA_PATH,
        help="The test raw data path.",
    )
    parser.add_argument(
        "--data_path", type=str, default=DATA_PATH, help="The raw data path."
    )
    parser.add_argument(
        "--I_dir", type=str, default=I_DIR, help="The training image path"
    )
    parser.add_argument(
        "--I_prime_dir", type=str, default=I_PRIME_DIR, help="The training image path"
    )
    parser.add_argument(
        "--pts1_file",
        type=str,
        default=PTS1_FILE,
        help="The training path to 4 corners on first image",
    )
    parser.add_argument(
        "--test_pts1_file",
        type=str,
        default=TEST_PTS1_FILE,
        help="The test path to 4 corners on second image",
    )
    parser.add_argument(
        "--num_data", type=int, default=DATA_NUMBER, help="The data size for training"
    )
    parser.add_argument(
        "--test_num_data",
        type=int,
        default=TEST_DATA_NUMBER,
        help="The data size for test",
    )
    parser.add_argument(
        "--gt_file", type=str, default=GROUND_TRUTH_FILE, help="The ground truth file"
    )
    parser.add_argument(
        "--test_gt_file",
        type=str,
        default=TEST_GROUND_TRUTH_FILE,
    ...
    parser.add_argument(
        ...
        help="File that contains all names of files",
    )
    parser.add_argument(
        "--test_filenames_file",
        type=str,
        default=TEST_FILENAMES_FILE,
        help="File that contains all names of files",
    )
    parser.add_argument(
        "--visual",
        type=str2bool,
        default="false",
        help="Visualize obtained images to debug",
    )
    parser.add_argument(
        "--artifact_mode",
        type=str,
        default="None",
        help="Add aftifacts to the images",
        choices=["noise", "None"],
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="N",
        help="Y: append to existing data. N: delete old data, create new data",
    )
    parser.add_argument(
        "--start_index",
        type=int,
        default=0,
        help="start_index of the new created sample",
    )
